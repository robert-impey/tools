...
func TestListManagedFolders_ListsExistingFolders(t *testing.T) {
	// 1. Setup Temp Directory
	tempDir := t.TempDir()
	...
	// 2. Create actual directories
	folderA := filepath.Join(loc1, "FolderA")
	folderB := filepath.Join(loc2, "FolderB")

	os.MkdirAll(folderA, 0755)
	os.MkdirAll(folderB, 0755)

	// 3. Initialize Manager
	// FolderC is included in the list but does not exist on disk
	fm := &FolderManager{
		Locations: []string{loc1, loc2},
		Folders:   []string{"FolderA", "FolderB", "FolderC"},
	}
	...
	absA, _ := filepath.Abs(folderA)
	absB, _ := filepath.Abs(folderB)

	// Check for expected matches
	if !strings.Contains(output, "Loc1") {
		t.Errorf("Output should contain 'Loc1', but was:\n%s", output)
	}
	if !strings.Contains(output, absA) {
		t.Errorf("Output should contain absolute path for FolderA: %s", absA)
	}
	if !strings.Contains(output, "Loc2") {
		t.Errorf("Output should contain 'Loc2'")
	}
	if !strings.Contains(output, absB) {
		t.Errorf("Output should contain absolute path for FolderB: %s", absB)
	}

	// Check that non-existent FolderC is NOT in the output
	if strings.Contains(output, "FolderC") {
		t.Error("Output should not contain 'FolderC' as it does not exist")
	}
}
